...
class AppCompatBackgroundHelper {
    ...
    void loadFromAttributes(AttributeSet attributeSet, int i) {
        attributeSet = TintTypedArray.obtainStyledAttributes(this.mView.getContext(), attributeSet, C0176R.styleable.ViewBackgroundHelper, i, 0);
        try {
            if (attributeSet.hasValue(C0176R.styleable.ViewBackgroundHelper_android_background) != 0) {
                this.mBackgroundResId = attributeSet.getResourceId(C0176R.styleable.ViewBackgroundHelper_android_background, -1);
                i = this.mDrawableManager.getTintList(this.mView.getContext(), this.mBackgroundResId);
                if (i != 0) {
                    setInternalBackgroundTint(i);
                }
            }
            if (attributeSet.hasValue(C0176R.styleable.ViewBackgroundHelper_backgroundTint) != 0) {
                ViewCompat.setBackgroundTintList(this.mView, attributeSet.getColorStateList(C0176R.styleable.ViewBackgroundHelper_backgroundTint));
            }
            if (attributeSet.hasValue(C0176R.styleable.ViewBackgroundHelper_backgroundTintMode) != 0) {
                ViewCompat.setBackgroundTintMode(this.mView, DrawableUtils.parseTintMode(attributeSet.getInt(C0176R.styleable.ViewBackgroundHelper_backgroundTintMode, -1), null));
            }
