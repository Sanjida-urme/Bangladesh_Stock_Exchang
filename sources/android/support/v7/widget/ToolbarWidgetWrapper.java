...
public class ToolbarWidgetWrapper implements DecorToolbar {
    ...
    public ToolbarWidgetWrapper(Toolbar toolbar, boolean z) {
        this(toolbar, z, C0176R.string.abc_action_bar_up_description, C0176R.drawable.abc_ic_ab_back_material);
    }
    ...
    public ToolbarWidgetWrapper(Toolbar toolbar, boolean z, int i, int i2) {
        ...
        toolbar = TintTypedArray.obtainStyledAttributes(toolbar.getContext(), null, C0176R.styleable.ActionBar, C0176R.attr.actionBarStyle, 0);
        this.mDefaultNavigationIcon = toolbar.getDrawable(C0176R.styleable.ActionBar_homeAsUpIndicator);
        if (z) {
            z = toolbar.getText(C0176R.styleable.ActionBar_title);
            if (!TextUtils.isEmpty(z)) {
                setTitle(z);
            }
            z = toolbar.getText(C0176R.styleable.ActionBar_subtitle);
            if (!TextUtils.isEmpty(z)) {
                setSubtitle(z);
            }
            Drawable drawable = toolbar.getDrawable(C0176R.styleable.ActionBar_logo);
            if (drawable == true) {
                setLogo(drawable);
            }
            drawable = toolbar.getDrawable(C0176R.styleable.ActionBar_icon);
            if (drawable == true) {
                setIcon(drawable);
            }
            ...
            setDisplayOptions(toolbar.getInt(C0176R.styleable.ActionBar_displayOptions, 0));
            z = toolbar.getResourceId(C0176R.styleable.ActionBar_customNavigationLayout, 0);
            if (z) {
                setCustomView(LayoutInflater.from(this.mToolbar.getContext()).inflate(z, this.mToolbar, false));
                setDisplayOptions(this.mDisplayOpts | 16);
            }
            z = toolbar.getLayoutDimension(C0176R.styleable.ActionBar_height, 0);
            if (z <= false) {
                LayoutParams layoutParams = this.mToolbar.getLayoutParams();
                layoutParams.height = z;
                this.mToolbar.setLayoutParams(layoutParams);
            }
            z = toolbar.getDimensionPixelOffset(C0176R.styleable.ActionBar_contentInsetStart, -1);
            int dimensionPixelOffset = toolbar.getDimensionPixelOffset(C0176R.styleable.ActionBar_contentInsetEnd, -1);
            if (z < false || dimensionPixelOffset >= 0) {
                this.mToolbar.setContentInsetsRelative(Math.max(z, 0), Math.max(dimensionPixelOffset, 0));
            }
            z = toolbar.getResourceId(C0176R.styleable.ActionBar_titleTextStyle, 0);
            if (z) {
                this.mToolbar.setTitleTextAppearance(this.mToolbar.getContext(), z);
            }
            z = toolbar.getResourceId(C0176R.styleable.ActionBar_subtitleTextStyle, 0);
            if (z) {
                this.mToolbar.setSubtitleTextAppearance(this.mToolbar.getContext(), z);
            }
            z = toolbar.getResourceId(C0176R.styleable.ActionBar_popupTheme, 0);
            if (z) {
        ...
        this.mToolbar.setNavigationOnClickListener(new C02161());
    }
    ...
    public void setMenu(Menu menu, MenuPresenter.Callback callback) {
        if (this.mActionMenuPresenter == null) {
            ...
            this.mActionMenuPresenter.setId(C0176R.id.action_menu_presenter);
        }
