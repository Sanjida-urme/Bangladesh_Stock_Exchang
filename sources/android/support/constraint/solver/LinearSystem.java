...
public class LinearSystem {
    ...
    private int enforceBFS(Goal goal) throws Exception {
        ...
        for (int i3 = 0; i3 < linearSystem.mNumRows; i3++) {
            if (linearSystem.mRows[i3].variable.mType != Type.UNRESTRICTED) {
                if (linearSystem.mRows[i3].constantValue < 0.0f) {
                    obj = 1;
                    break;
                }
            }
        ...
        for (i2 = 0; i2 < linearSystem.mNumRows; i2++) {
            if (linearSystem.mRows[i2].variable.mType != Type.UNRESTRICTED) {
                if (linearSystem.mRows[i2].constantValue < 0.0f) {
                    break;
                }
            }
        }
