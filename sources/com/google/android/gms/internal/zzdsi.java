...
public final class zzdsi {
    ...
    public static ECParameterSpec zza(zzdsk zzdsk) throws NoSuchAlgorithmException {
        String str;
        String str2;
        String str3;
        String str4;
        String str5;
        switch (zzdsj.zzluz[zzdsk.ordinal()]) {
            case 1:
    ...
    public static void zza(ECPoint eCPoint, EllipticCurve ellipticCurve) throws GeneralSecurityException {
        ...
        if (affineX != null) {
            if (affineY != null) {
                if (affineX.signum() != -1) {
                    if (affineX.compareTo(zza) == -1) {
                        if (affineY.signum() != -1) {
                            if (affineY.compareTo(zza) == -1) {
    ...
    public static byte[] zza(EllipticCurve ellipticCurve, zzdsl zzdsl, ECPoint eCPoint) throws GeneralSecurityException {
        zza(eCPoint, ellipticCurve);
        int bitLength = (zza(ellipticCurve).subtract(BigInteger.ONE).bitLength() + 7) / 8;
        Object obj;
        switch (zzdsj.zzluy[zzdsl.ordinal()]) {
            case 1:
                int i = (bitLength * 2) + 1;
                obj = new byte[i];
                Object toByteArray = eCPoint.getAffineX().toByteArray();
                Object toByteArray2 = eCPoint.getAffineY().toByteArray();
            ...
            case 2:
                ...
                System.arraycopy(obj, 0, obj2, bitLength - obj.length, obj.length);
                obj2[0] = (byte) (eCPoint.getAffineY().testBit(0) ? 3 : 2);
                return obj2;
            default:
                String valueOf = String.valueOf(zzdsl);
